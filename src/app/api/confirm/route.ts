...
export async function GET(req: Request) {
  ...
  try {
    ...
    const filters = email ? { confirmation_token: token, email } : { confirmation_token: token };
    ...
    if (findErr) return NextResponse.json({ ok: false, error: "DB_FIND_ERROR" }, { status: 500 });
    if (!lead) return NextResponse.json({ ok: false, action: "confirm", token, email, error: "TOKEN_NOT_FOUND" }, { status: 404 });
    ...
    if (updErr) return NextResponse.json({ ok: false, error: "DB_UPDATE_ERROR" }, { status: 500 });
